...
x.domain(dimensions = d3.keys(indicators[0]).filter(function(d) {
 ...
 // console.log(d[0]);
 if(d == "AgeGrpNumeric" || d == "BMI" || d == "SleepTime")//|| d == "RaceNumeric" || d=="GenderNumeric"
 {
...
function brushend() {
  d3.selectAll('#tblChart')
  .remove();    
filteredData = filerBrushData(indicators,extentsGlobal, isAgeGrp,isBMI,isSleepTime);
...
tabulate(arrObj,['AgeGrpNumeric','AgeGrp', 'TotalCount','BMIRange','SleepTime' ]);
...
filerBrushData = function(data,extents,isAgeGrp,isBMI,isSleepTime){
var filteredData = [];
